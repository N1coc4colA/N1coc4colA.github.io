...
#define CAMERA_HEIGHT 4.5 // how high the camera should be
...
vec3 getRay(vec2 fragCoord) {
   vec2 uv = (fragCoord.xy / iResolution.xy) * 2.0 - 1.0; // Normalize coordinates
   uv *= vec2(iResolution.x / iResolution.y, 1.0);       // Aspect ratio correction
   vec3 proj = normalize(vec3(uv, -1.0));               // Point downward
   return proj;
}
...
vec3 getSunDirection() {
 return normalize(vec3(0.0, 1.0, 0.0)); // Sun directly overhead
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
 ...
 vec3 origin = vec3(0, CAMERA_HEIGHT, 0);
...

{
    ...
    function resizeCanvas() {
        canvas.width = window.innerWidth;  // Set width to window's inner width
        canvas.height = window.innerHeight; // Set height to window's inner height
    ...
    function draw() {
        ...
        ctx.fillRect(0, 0, canvas.width, canvas.height); // Draw rectangle filling the canvas
    }
